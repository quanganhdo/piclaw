...
### Compaction (order 13)
- **Processing method** selector: **Selective** (default) or **Pipelined**. The local method is captured once per compaction generation; both choices share lifecycle, provider/auth resolution, progressive execution, exact-boundary handling, and pruning. See [Smart-compaction processing method](configuration.md#smart-compaction-processing-method), [Compaction models and deadlines](compaction-models-and-deadlines.md), and the complete [Pipelined smart-compaction guide](pipelined-compaction.md).
- **Compaction model** selects an optional strict `provider/model` for local smart compaction; empty uses the active model. The single compaction deadline covers deterministic preparation, provider connection/prefill, streaming, validation, and settlement.
