...
| [Index process](../../runtime/src/workspace-index-process.ts) | Background process, active-child guard, matching expected-mode argument; parent supplies workspace/store/data environment | Bind and verify parent-captured roots/database in the child; integrate bounded dirty work and overdue reconciliation |
...
Absence of execution identity is not a trust flag. The only admissible no-identity query/get path is the future `execute` closure registered for each tool in `runtime/src/extensions/memory-search.ts`, invoked through the existing tool dispatcher with a real extension context and captured chat binding. It must check active tool grants and configured single-user mode before calling a module-private content reader. Do not export a raw no-identity query/get API or accept a request field selecting internal trust. The only no-identity writer is a private coordinator operation reached through `runWorkspaceIndexProcessFromArgs` after the proposed mode/root/database admission checks described below; startup, mutation, Dream and tool handlers may enqueue that operation, not invoke an alternate writer. Rebuild is an explicitly reviewed internal maintenance invocation through that same coordinator, without a new HTTP route. These are proposed wiring restrictions for #376/#387; the current generic index helper's absence-of-identity check is insufficient on its own. Preserve existing tool admission and apply the memory bootstrap's stricter identity/default-owner/chat checks when an execution identity is present.

The current worker checks exactly one matching `--expected-mode`; its parent supplies workspace/store/data environment variables. It does **not** independently verify a parent-captured root tuple. The current `workspaceIndexAccess` guard pins mode/workspace/execution identity, not store/data/database. #376 must add a parent-created binding containing canonical workspace/store/data roots, database location/identity and access mode, pass it over a coordinator-owned startup channel, and compare it with independently resolved child configuration before opening or writing the note index. A query/tool argument or CLI flag cannot opt into this writer authority. Environment values supplied to the child alone are not independent verification. Root substitution tests belong to NR-A01/A03; none of these new checks is claimed as existing protection.
...
A query captures one committed database generation plus the namespace/scope dirty generation, coverage-state version and last-complete reconciliation timestamp before candidate selection. Every known note mutation, including a new or non-candidate path, advances the scope dirty generation; per-file counters alone are insufficient. It then validates source bytes for the bounded candidate set, groups work by path, and returns only candidates matching their stored revision and admitted file identity. Derive the snippet from that verified slice.

Immediately before delivery, recheck namespace/access, affected per-file generations **and** the captured scope dirty generation, coverage-state version, published-generation pointer and reconciliation age. Read these final markers from current committed state, not the query's older SQLite read snapshot. Apply this check even to a zero-hit query. Any observed scope-generation, coverage-state or published-generation change makes the result `partial` with `refresh_pending`, even if another worker has since restored ready status; alternatively restart candidate selection once if the original work/deadline budgets permit. A reconciliation deadline crossed during the query also requires `partial`. Do not reuse hits across that retry without revalidation. Per-file changes withhold affected text; namespace/access loss or index corruption discards the whole response. No awaited I/O may intervene between the final classification and return. No synchronous full scan or note-text write occurs on the ordinary query path.
...
| `not_found` | Get only: well-formed reference has no live row in this namespace; includes pruned/deleted/moved references; no historical tombstone service or cross-namespace lookup |
| `source_stale` | Get only: row exists but bytes/revision changed, source disappeared, or filesystem admission (path, regular-file/link/size checks) failed; return no chunk text, invalidate that file and request reconciliation. Queries instead use `partial` with the `source_stale` reason |
...
| `source_unavailable` | Get: transient permission/I/O/read instability prevented source validation, with no chunk text. Query: omit that file and return `partial` with the `source_unavailable` reason |
...
Known dirty files are excluded until refreshed. A query may return other verified hits with `completeness: partial` and a closed reason set (`refresh_pending`, `excluded_sources`, `validation_budget`, `source_stale`, `source_unavailable`). Reasons may coexist and must be deduplicated. `excluded_sources` covers known admission/chunk-policy exclusions; `source_stale` specifically records candidates invalidated by query-time revision, disappearance or filesystem-admission checks. A query with one stale candidate and other valid hits returns only those valid hits as `partial`, including `source_stale`. If all candidates are stale it returns `partial`, an empty hit list and `source_stale`; never top-level `source_stale`, `not_found` or complete `ok`. Zero candidates plus a changed scope generation likewise returns empty `partial` with `refresh_pending`. Zero hits under incomplete coverage is not a definitive no-match. Other file failures never relax admission. Query-wide authority loss or database corruption discards all pending output. No malformed-SQL fallback into a broader corpus; plain-language query preparation can reuse the existing FTS helpers.
