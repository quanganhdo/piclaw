...
The assessment produced this ADR, its evidence tables and a proposed semantic contract suite. It specifies a test implementation of the selected Earendil public contracts because released `0.84.1` is incomplete and draft PR #8076 still has no concrete public v3 harness constructor/runtime. It does not implement the production runner, change persistence, replay archived fixes or deploy a new runtime.
...
- Earendil owns transcript execution, model/tool lifecycle, execution compaction and execution recovery. Harness v3's entries/registers/usage-ledger design is the target execution model; released `0.84.1` and draft PR #8076 are evidence, not selected dependencies.
...
- Production remains on the v2.13.2 loop and released Earendil `0.84.1` until one coherent tagged harness release and the staged migration pass the documented gates.
