...
| CAP-023 | Execute tools | SDK AgentSession with Piclaw tool registry | Validates/adopts tools, emits lifecycle, runs local/remote effects | Tool result appended or error | Piclaw patches tool admission and tracks parallel calls externally | Earendil tool lifecycle and process-local `EffectGate`; Piclaw supplies direct tools/context, never duplicate execution | Proven |
