...
| Released packages | `@earendil-works/pi-coding-agent`, `pi-agent-core`, `pi-ai` and `pi-tui` remain at `0.84.1` |
...
Keep released `0.84.1` in production as the implemented baseline. Use the specification on Earendil `main` as the authoritative design and PR #8076 only as pinned draft implementation evidence. Prepare Piclaw against the public lower-level v3 contracts, but do not update package pins or start production migration until one coherent tagged release passes every gate above.
