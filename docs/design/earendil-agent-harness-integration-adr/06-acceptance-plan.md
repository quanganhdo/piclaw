...
|---|---|---|
| Which Earendil source/version should production target? | Released `0.84.1` is baseline evidence only. The `main` Harness v3 specification is authoritative; draft PR #8076 at `fd389abc4677b4e0fa5dc9b2bbd2e63418f079b4` contains substantial storage/primitives but no concrete public harness runtime. | Keep production on released `0.84.1`; select one coherent tagged release only after required runtime/backend slices and all migration gates pass. |
