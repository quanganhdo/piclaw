...
export function ScheduledTasksSection() {
  ...
  async function action(name: string, options: Record<string, unknown> = {}) {
    ...
    if (!confirm(options.confirm_zero_budget === true ? 'A zero task cap blocks model execution. Disable the cap instead for no task-specific limit. Apply zero anyway?' : `${label} for ${selected.id}?`)) return;
    setBusy(true);
    setError("");
    try {
  ...
  return <section className="settings-panel__section settings-panel__section--scheduled-tasks">
    ...
    {!loading && tasks.length > 0 && <div className="settings-panel__scheduled-layout">
      ...
      {selected && <article className="settings-panel__scheduled-detail">
        ...
        {selected.task_kind === "agent" && <form className="settings-panel__scheduled-budget" onSubmit={(event) => { event.preventDefault(); void action("set_budget", { budget_usd: budget, enabled: true, ...(selected.budget_cap_revision != null ? { confirm_revision: selected.budget_cap_revision } : {}), ...(/^0(?:\.0{1,6})?$/.test(budget.trim()) ? { confirm_zero_budget: true } : {}) }); }}>
        ...
        <p className="settings-panel__description">This editor is the single source of truth for the task cap. No task cap permits runs when no other limits apply. Zero blocks model execution. Instance/provider limits still apply; caps reset per run and are not reservations.</p>
        {selected.budget_readiness && <div role="status" className="settings-panel__description"><strong>Budget: {selected.budget_readiness.status}</strong> · checked {selected.budget_readiness.checked_at}<p>{selected.budget_readiness.summary}</p>{selected.budget_readiness.blockers.map((blocker: any) => <div>{blocker.scope}: {blocker.reason}</div>)}{selected.budget_readiness.next_steps.map((step: string) => <div>{step}</div>)}</div>}
