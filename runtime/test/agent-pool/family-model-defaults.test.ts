...
test('family active model HTTP is owner-scoped, safe-projected and revalidates on its chat lane', async () => {
  ...
  const calls:any[]=[]; let revokeOnLane=false;
  const modelState:any={current:'family-test/a',models:['family-test/a'],model_options:[{label:'family-test/a',provider:'family-test',id:'a',name:'Model A',context_window:200000,pricing:null,reasoning:true,thinking_levels:['off','high'],thinking_level_labels:['Off','High']}],thinking_level:'high',thinking_level_label:'High',supports_thinking:true,available_thinking_levels:['off','high'],available_thinking_level_labels:['Off','High'],provider_usage:{secret:'hidden'},latest_requested_model:null,latest_response_model:null,scoped_models_only:true,scoped_model_filter_active:true,enabled_model_patterns:['family-test/*'],provider_diagnostics:{providers:[{auth_label:'hidden'}],registered_provider_ids:[],composition_error:null}};
  const agentPool:any={getAvailableModels:async(jid:string,options:any)=>{calls.push(['get',jid,options]);return modelState;},getContextUsageForChat:async()=>({tokens:100,contextWindow:200000,percent:1}),applyOwnedModelControl:async(jid:string,command:any)=>{calls.push(['apply',jid,command]);return{status:'success',message:'ok',model_label:'family-test/a'};},isActive:()=>false};
  const queue={enqueue:(task:()=>unknown,_key:string,lane:string)=>{calls.push(['lane',lane]);if(revokeOnLane)getDb().query('DELETE FROM web_sessions WHERE session_id=?').run(alice.authentication.sessionId!);void task();}};
  const router=new RequestRouterService({json,authGateway,agentPool,queue} as any,'family-shared');
  const req=(path:string,method='GET',body?:unknown,token=`token-${alice.userId}`,headers:Record<string,string>={})=>router.handle(new Request('https://local'+path,{method,headers:{cookie:`piclaw_session=${token}`,origin:'https://local','x-piclaw-account-id':alice.userId,'x-piclaw-login-id':alice.authentication.sessionId!,...headers},...(body===undefined?{}:{body:JSON.stringify(body)})}));
  const listed=await req('/agent/models?chat_jid='+encodeURIComponent(alice.homeChatJid!));expect(listed.status).toBe(200);const payload=await listed.json();expect(payload.model_options[0]).toMatchObject({label:'family-test/a',context_window:200000});expect(JSON.stringify(payload)).not.toMatch(/provider_usage|provider_diagnostics|enabled_model_patterns|auth_label|secret/);expect(calls[0][2]).toEqual({includeProviderUsage:false,includeProviderDiagnostics:false});
  const changed=await req('/agent/models?chat_jid='+encodeURIComponent(alice.homeChatJid!),'PATCH',{action:'model',value:'family-test/a'});expect(changed.status).toBe(200);expect(calls).toContainEqual(['lane',`chat:${alice.homeChatJid}`]);expect(calls.find(row=>row[0]==='apply')?.[2]).toMatchObject({type:'model',provider:'family-test',modelId:'a'});
  for(const [path,method,body,headers] of [[`/agent/models?chat_jid=${encodeURIComponent(bob.homeChatJid!)}`,'GET',undefined,{}],[`/agent/models?chat_jid=${encodeURIComponent(alice.homeChatJid!)}&user_id=${bob.userId}`,'GET',undefined,{}],[`/agent/models?chat_jid=${encodeURIComponent(alice.homeChatJid!)}`,'POST',undefined,{}],[`/agent/models?chat_jid=${encodeURIComponent(alice.homeChatJid!)}`,'PATCH',{action:'model',value:'family-test/a'},{origin:'https://foreign'}]] as any[])expect((await req(path,method,body,undefined,headers)).status).toBe(403);
  expect((await req(`/agent/models?chat_jid=${encodeURIComponent(alice.homeChatJid!)}`,'PATCH',{action:'model',value:'family-test/a',user_id:bob.userId})).status).toBe(400);
  revokeOnLane=true;expect((await req('/agent/models?chat_jid='+encodeURIComponent(alice.homeChatJid!),'PATCH',{action:'thinking',value:'high'})).status).toBe(403);expect(calls.filter(row=>row[0]==='apply')).toHaveLength(1);
});
