...
const paths=['docs/multi-user/user-guide.md','docs/multi-user/administrator-guide.md','docs/multi-user/troubleshooting.md','docs/multi-user/migration-copy.md','docs/multi-user/operator-recovery.md','docs/multi-user/README.md','docs/multi-user/scheduled-execution.md','docs/multi-user/memory-bootstrap.md','docs/multi-user/family-acceptance.md'];
...
test('family guides cover user controls, destructive effects and gated operator workflows',()=>{
  ...
  expect(user).toContain('"mode": "family-shared"');expect(user).toContain('family-acceptance.md');
  const acceptance=read('docs/multi-user/family-acceptance.md');
  for(const text of ['2,472 passed; 0 failed','459.57s','1,387.6 MiB','4 vCPUs','4.0 GiB RAM','no swap','33.9%','No live workspace was migrated or activated'])expect(acceptance).toContain(text);
  for(const image of ['01-rich-conversation-desktop.png','02-curated-model-picker.png','03-owned-session-picker.png','04-live-status-thinking-tools-queue.png','05-rich-conversation-mobile.png','06-family-administration.png','07-workspace-security.png'])expect(acceptance).toContain(`screenshots/${image}`);
